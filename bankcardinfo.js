...
// copy from https://github.com/navyxie/bankcardinfo/blob/master/README.md and change
  ...
  function _getBankInfoByCardNoAsync(cardNo, cbf) {
    ...
    _getBankInfoByCardNo(cardNo, function(err, info) {
      if (!err && info) {
        ...
        if (jQuery) {
          jQuery
          .get("https://ccdcapi.alipay.com/validateAndCacheCardInfo.json?_input_charset=utf-8&cardNo=" + cardNo + "&cardBinCheck=true").then((res) => {
            // console.log('res', res);
            if (res.validated) {
              var info = {};
              info['bankName'] = getBankNameByBankCode(res.bank);
              info['cardType'] = res.cardType;
              info['bankCode'] = res.bank;
              info['cardTypeName'] = getCardTypeName(res.cardType);
              info['backName'] = info['bankName']; //向下兼容，修改字段错别字
              cbf(null, info);
            } else {
              console.log('cardNo res', res);
              if (res && res.messages && res.messages[0] && res.messages[0].errorCodes) {
                errMsg = cardNo + ":该银行卡不存在," + res.messages[0].errorCodes;
              }
              else {
                errMsg = cardNo + ":该银行卡不存在," + JSON.stringify(res);
              }
              cbf(errMsg);
            }
          }).catch((rej) => {
